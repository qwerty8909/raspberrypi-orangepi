        ...
        for plate in [carplate, carplate_res]:
            ...
            for psm in [6, 13]:
                ...
                text = pytesseract.image_to_string(plate,
                                                   config=f'-l rus --psm {psm} --oem 3 -c tessedit_char_whitelist'
                                                          f'=АВЕКМНОРСТУХ0123456789')
...
# формируем поток кадров 640x360 и 30 кадров в секунду
...
# выбираем необходимый (10) для вывода (30/10 = 3 кадра в секунду)
cap = 1
frameRate = 10
while True:
